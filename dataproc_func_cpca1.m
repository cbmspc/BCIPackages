...
function DRmatC = dataproc_func_cpca(TrainData, TrainLabels, varargin)
...
for i = 1:2:length(varargin)
    for c = 1:Nclass
        if strcmp(varargin{i},strcat('DRparm',int2str(c))) && ~isempty(varargin{i+1}) && isnumeric(varargin{i+1})
            DRparm(c) = varargin{i+1};
        end
    end
end
...
for c = 1:Nclass
    ...
    elseif DRparm(c) < 0 && DRparm(c) > -1
        coeffrC{c} = coeff(:, (latent/max(latent) > -DRparm(c)) );
    elseif DRparm(c) <= -1
        % Undefined behavior. Use default
        coeffrC{c} = coeff(:, latent > mean(latent(latent>0)) );
    elseif DRparm(c) < 1
        % Keep fraction of the sum of eigenvalues
        coeffrC{c} = coeff(:, 1:find(cumsum(latent)/sum(latent) >= DRparm(c),1) );
    else
        % Keep the number of eigenvectors
        % This number cannot exceed the number of non-zero eigenvalues
        coeffrC{c} = coeff(:, 1:min(nnz(latent>0),DRparm(c)) );
    end
