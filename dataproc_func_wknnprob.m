% Weighted (by inverse distance) K-nearest neighbor probability
% Returns the probability that "wantlabel" is the predicted label of "test"
% data given "train" training data and "K" neighbors

function Probability = dataproc_func_wknnprob (test, train, label, K, wantlabel)
...
for c = 1:Nclass
    for t = 1:Ntest
        classscore(t,c) = sum(1./ Y(t, find( J(t,:) == labelnames(c) ) ));
    end
end
