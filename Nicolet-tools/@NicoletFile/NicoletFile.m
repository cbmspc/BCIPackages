...
%           Index(curIdx + i).sectionL = fread(h,1, 'uint32');  
    ...
    % Po 20171019: cache the indexInstances into a file
    fileinfo = dir(filename);
    CacheFile = [filename '.cache.mat'];
    CacheRecalculated = 0;
	PersistCache = [];
    if exist(CacheFile, 'file')
        try
            load(CacheFile, 'PersistCache');
        catch
            try
                delete(CacheFile);
            end
    ...
    if isempty(PersistCache) || ~strcmp(PersistCache.filename,filename) || PersistCache.filebytes ~= fileinfo.bytes || PersistCache.filedatenum ~= fileinfo.datenum || PersistCache.nrDynamicPackets ~= nrDynamicPackets || isempty(PersistCache.indexInstances)
        PersistCache.filename = filename;
        PersistCache.filebytes = fileinfo.bytes;
        PersistCache.filedatenum = fileinfo.datenum;
        PersistCache.nrDynamicPackets = nrDynamicPackets;
        PersistCache.indexInstances = cell(1,nrDynamicPackets);
    end
    for i = 1: nrDynamicPackets
        ...
        if isempty(PersistCache.indexInstances) || ~isstruct(PersistCache.indexInstances{i})
            indexInstances = Index([Index.sectionIdx] == infoIdx);
            PersistCache.indexInstances{i} = indexInstances;
            CacheRecalculated = 1;
        else
            indexInstances = PersistCache.indexInstances{i};
        end
    ...
    if CacheRecalculated
        try
            save('-v7.3', CacheFile, 'PersistCache');
        end
    end
...
%       reserved = fread(h, 3, 'uint16'); %#ok<NASGU>
      ...
      for iSeg = 1:length(obj.segments)
          ...
          % Po
          if iSeg <= length(tsPackets)
              tsPacket = tsPackets(iSeg);
          else
    ...
    function out = getdata(obj, segment, range, chIdx)
      ...
      for i = 1 : lChIdx
        ...
        if lastSection > lastSectionForSegment 
            % Po
          warning('Index out of range for current section: %i > %i, on channel: %i', ... 
            range(2), cSectionLengths(lastSectionForSegment+1), chIdx(i));
        end
        
        try
            tmp = lastSection;
            if tmp > numel(allSections)
                tmp = numel(allSections);
            end
            useSections = allSections(firstSection: tmp) ;
            useSectionL = sectionLengths(firstSection: tmp) ;
        catch me
            keyboard
        end
        ...
        if length(useSections) > 1
          ...
          try
              fseek(h, curSec.offset,'bof');
              tmp = fread(h, length(out)-curIdx + 1, 'int16') * mult;
              tmpb = curIdx + length(tmp) - 1;
              out(curIdx : tmpb,i) = tmp;
          catch me
              keyboard
          end
        end
