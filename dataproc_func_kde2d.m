...
while abs(val)>10^-5
    ...
    c=c+1; if c>10^3, error('Algorithm failed to converge in 1000 iterations'), end
end
...
a_t=exp(-(0:n-1)'.^2*pi^2*t_y/2)*exp(-(0:n-1).^2*pi^2*t_x/2).*a; %transpose goes with y coord.
